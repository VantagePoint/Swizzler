 ...
 * @{
 */
GD_C_API GD_FILE* (*orig_GD_fopen)(const char* filename, const char* mode);
GD_C_API GD_FILE* replaced_GD_fopen(const char* filename, const char* mode)
{
	NSLog(@"GD_C_API fopen");
	return orig_GD_fopen(filename, mode);
};
...
void GD_C_FileSystem_hooks()
{
	void *GD_fopen_sub = MSFindSymbol(NULL, "_GD_fopen");
	MSHookFunction((void *)GD_fopen, (void *)replaced_GD_fopen, (void**)&orig_GD_fopen);
